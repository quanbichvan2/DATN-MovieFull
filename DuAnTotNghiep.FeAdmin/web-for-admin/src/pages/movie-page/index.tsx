...
const MovieTable: React.FC = () => {
  ...
  const filteredMovies = currentMovies.filter((movie) =>
    movie.title.toLowerCase().includes(searchTerm.toLowerCase()) && !movie.isHidden
  );
  ...
  // const hideMovie = async (movieId: string) => {
  //   try {
  //     // Tạo đối tượng movieData với trường isHidden = true
  //     const movieData = { isHidden: true };

  //     // Gọi API updateMovie với movieId và movieData
  //     const updatedMovie = await updateMovie(movieId, movieData);
  //     console.log('Movie updated successfully:', updatedMovie);
  //     // Sau khi cập nhật, bạn cần tải lại danh sách phim để hiển thị thay đổi
  //     fetchMovies(currentPage); // Đảm bảo làm mới dữ liệu phim
  //   } catch (error) {
  //     console.error('Error hiding movie:', error);
  //     setAlertMessage('Có lỗi xảy ra khi ẩn phim.');
  //     setAlertVariant('danger');
  //   }
  // };
                        ...
                        filteredMovies.map((movie) => (
                          <tr key={movie.id}>
                            ...
                            <td className="controllmovie">
                              ...
                              {/* <button
                                type="button"
                                className="btn btn-outline-danger gap-btn"
                                onClick={() => hideMovie(movie.id)}  // Chỉnh sửa ở đây
                              >
                                Dừng chiếu
                              </button> */}
                            </td>
